...
class ExportWorker(QThread):
    ...
    def __init__(self, sl: SheetSlice, chart_specs: list[ChartSpec], target_path: str, parent=None):
        super().__init__(parent)
        self._service = ExcelExportService()
        self._sl = sl
        self._specs = chart_specs
    ...
    def run(self) -> None:
        try:
            path = self._service.export(self._sl, self._specs, self._path,
...
class ExportDialog(QDialog):
    def __init__(self, sl: SheetSlice, chart_specs: list[ChartSpec], default_path: str, parent=None):
        super().__init__(parent)
        self.setWindowTitle("📈 导出散点图文件")
        ...
        self.info = QLabel(
            f"Sheet1 = raw data（原始数据副本，{self._sl.row_count} 行）\n"
            f"其后 {n_charts} 张图各占一个 Sheet：{', '.join(s.chart_name for s in chart_specs[:4])}"
            + ("…" if n_charts > 4 else "")
            + "\n覆盖已存在文件前自动备份；取消后自动回滚。"
        )
    ...
    def _start(self) -> None:
        ...
        self._worker = ExportWorker(self._sl, self._specs, target, self)
